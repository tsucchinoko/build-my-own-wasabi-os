...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    let mut vram = init_vram(efi_system_table).expect("init_vram_failed");
    let vw = vram.width;
    let vh = vram.height;
    fill_rect(&mut vram, 0x000000, 0, 0, vw, vh).expect("fill_rect_failed");
    fill_rect(&mut vram, 0xff0000, 32, 32, 32, 32).expect("fill_rect_failed");
    fill_rect(&mut vram, 0x00ff00, 64, 64, 64, 64).expect("fill_rect_failed");
    fill_rect(&mut vram, 0x0000ff, 128, 128, 128, 128)
        .expect("fill_rect_failed");
    for i in 0..256 {
    ...
    for i in (0..=rect_size).step_by(grid_size as usize) {
        ...
        let _ = draw_line(&mut vram, 0x00ff00, i, 0, i, rect_size);
    }
    let cx = rect_size / 2;
    let cy = rect_size / 2;
    for i in (0..=rect_size).step_by(grid_size as usize) {
...
trait Bitmap {
    ...
    /// # Safety
    ///
    /// Returned pointer is valid as long as the givien coodinates are valid.
    /// which means that passing is in_*_range tests.
